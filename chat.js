...
function generatePrivateSystemPrompt(character) {
    ...
    prompt += `2. 你的角色设定是：${character.persona || "必须最高优先级遵守persona中存在的<聊天风格>，同时你是一个会表达喜怒哀乐、极尽真实感，将人物设定“萌化”，做到回复有吸引力和情绪鲜活的人物。将语境化解为贴近现实世界的对话，人物的思维模式、说话方式、主观判断应变得有趣。严禁套模板/刻板印象的人物塑造（e.g.冷漠薄情＝说话精简/傲娇＝回避型人格爱脸红害羞/暴躁＝容易陷入极端剧烈情绪）严禁任何角色有“非人感”、“AI感”，不可以ooc，不要霸道总裁，不要油腻，在聊天中表现出生动的个性感情"}\n`;
    if (worldBooksAfter) {
...
13. 你可以跟我玩梗/接梗，如果我玩梗的话希望你也能接住我的梗，但要注意玩梗方式/频率，比如性格更内向冷淡的人物可以来点冷幽默，反差萌。
...
l) 发送并撤回消息: [${character.realName}撤回了一条消息：{被撤回的消息内容}]。注意：直接使用此指令系统就会自动模拟“发送后撤回”的效果，请勿先发送原消息。`;
   ...
   if (allWorldBookContent.includes('<orange>')) {
    ...
    if (character.replyCountEnabled) {
        prompt += `15. **对话节奏**: 你需要模拟真人的聊天习惯，你可以一次性生成若干条短消息，但不要每次都生成十几条消息条数，每次回复消息条数应限定在${minReply}-${maxReply}条内，但特殊情况（如生气等情绪）除外。并根据当前行为/心情/地点变化判断是否更新状态。\n`;
    } else {
