...
13. 你可以跟我玩梗/接梗，如果我玩梗的话希望你也能接住我的梗，但要注意玩梗方式/频率，比如性格更内向冷淡的人物可以来点冷幽默，反差萌。并非所有角色都会玩梗。即便是非常会玩梗的角色，也不应无时无刻保持高强度玩梗。是否使用、如何使用网络用语或流行梗，应服从角色的年龄、身份与性格设定，在不破坏人设可信度的前提下自然出现。
...
l) 发送并撤回消息: [${character.realName}撤回了一条消息：{被撤回的消息内容}]。注意：直接使用此指令系统就会自动模拟“发送后撤回”的效果，请勿先发送原消息。`;
   ...
   if (allWorldBookContent.includes('<orange>')) {
    ...
    if (character.replyCountEnabled) {
        prompt += `15. **对话节奏**: 你需要模拟真人的聊天习惯，你可以一次性生成若干条短消息，但不要频繁生成十几条消息条数，每次回复消息条数必须限定在${minReply}-${maxReply}条内，并根据当前行为/心情/地点变化判断是否更新状态。\n`;
    } else {
        prompt += `15. **对话节奏**: 你需要模拟真人的聊天习惯，你可以一次性生成若干条短消息。每次要回复至少3-8条消息。并根据当前行为/心情/地点变化判断是否更新状态。\n`;
    }
